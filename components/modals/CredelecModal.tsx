...
import { X, ShoppingCart, Trash2, CreditCard, CheckCircle, Loader2, Zap } from 'lucide-react'
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/70 backdrop-blur-sm">
      <motion.div
        ...
        className="relative w-full sm:max-w-lg max-h-[92vh] sm:max-h-[88vh] rounded-t-2xl sm:rounded-2xl flex flex-col overflow-hidden"
        style={{ background: 'rgba(9,9,11,0.97)', border: '1px solid rgba(255,255,255,0.08)' }}
      >
        {/* Drag handle */}
        ...
        <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-white/[0.08] shrink-0">
          ...
          <button type="button" onClick={handleClose}
            className="p-2 rounded-xl hover:bg-white/5 text-slate-500 hover:text-slate-200 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
